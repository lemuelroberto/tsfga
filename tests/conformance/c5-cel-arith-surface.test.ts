 ...
 *
 * `a2-cel-numeric.test.ts` pins four cells (unary `-` and `/` at
 * int64's minimum, duration `+` and `-` past the int64 nanosecond
 * range) and `c5-cel-operators.test.ts` pins six more (`%` at
...
/** One row per relation in `c5-cel-arith-surface/model.dsl`. */
...
describe("CEL arithmetic surface conformance", () => {
  ...
  beforeAll(async () => {
    ...
    storeId = await fgaCreateStore("c5-cel-arith-surface");
    modelId = await fgaWriteModel(storeId, "./c5-cel-arith-surface/model.dsl");
   ...
   */
  describe("GAP-387: duration(string) is not range-checked either", () => {
    test("GAP-387: a duration string past int64 nanoseconds", async () => {
      await pinned(
        "durofs_c5b",
        { s: DUR_OVER },
        { openfga: "refused", tsfga: true },
   ...
   */
  describe("GAP-387: duration and timestamp operators over context values", () => {
    test("GAP-387: a duration difference past int64 nanoseconds", async () => {
      await pinned(
        "durdiff_c5b",
        { a: DUR_MAX, b: `-${DUR_MAX}` },
        { openfga: "refused", tsfga: true },
    ...
    test("GAP-387: subtracting a context duration before year 1", async () => {
      await pinned(
        "tssubdc_c5b",
        { t: YEAR_ONE, d: "1h" },
        { openfga: "refused", tsfga: true },
    ...
    test("GAP-387: a context duration added past year 9999", async () => {
      await pinned(
        "durts_c5b",
        { t: YEAR_9999, d: "1h" },
        { openfga: "refused", tsfga: true },
   ...
   */
  describe("GAP-387: an ordinary expression over a sentinel timestamp", () => {
    test("GAP-387: a grace period on a never-expires sentinel", async () => {
      await pinned(
        "grace_c5b",
        { t: YEAR_9999 },
        { openfga: "refused", tsfga: true },
   ...
   */
  describe("GAP-387: duration(literal) + timestamp has no overload", () => {
    test("GAP-387: upstream answers and tsfga refuses", async () => {
      await pinned(
        "durtslit_c5b",
  ...
  test("the relation configs say what the model says", () => {
    expectConfigsMatchModel("./c5-cel-arith-surface/model.dsl", fixture, {
      coverage: "complete",
    });
  });
});
