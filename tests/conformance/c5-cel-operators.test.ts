 ...
 *   three. `a2-cel-numeric.test.ts` has a `t + duration(…)` cell
...
/** One row per relation in `c5-cel-operators/model.dsl`. */
...
describe("CEL operator range-check conformance", () => {
  ...
  beforeAll(async () => {
    ...
    storeId = await fgaCreateStore("c5-cel-operators");
    modelId = await fgaWriteModel(storeId, "./c5-cel-operators/model.dsl");
   ...
   * shape so the neighbour sits beside it. `a2-cel-numeric` owns
   * the pin itself.
   */
  ...
  test("the relation configs say what the model says", () => {
    expectConfigsMatchModel("./c5-cel-operators/model.dsl", fixture, {
      coverage: "complete",
    });
  });
});
