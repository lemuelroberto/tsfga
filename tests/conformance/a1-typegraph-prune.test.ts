...
describe("a1: type-graph reachability", () => {
  ...
  beforeAll(async () => {
    ...
    storeId = await fgaCreateStore("a1-typegraph-prune");
    authorizationModelId = await fgaWriteModel(
      storeId,
      "./a1-typegraph-prune/model.dsl",
    );
    await fgaWriteTuples(
      storeId,
      "./a1-typegraph-prune/tuples.yaml",
  ...
  test("the relation configs say what the model says", () => {
    expectConfigsMatchModel("./a1-typegraph-prune/model.dsl", fixture, {
      coverage: "complete",
    });
  });
});
