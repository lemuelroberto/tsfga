...
// Deliberate probes of the type-graph reachability prune added in
// round 1 (`packages/core/src/type-graph.ts`), which answers
...
describe("b2: reachability the prune could lose", () => {
  ...
  beforeAll(async () => {
    ...
    storeId = await fgaCreateStore("b2-prune");
