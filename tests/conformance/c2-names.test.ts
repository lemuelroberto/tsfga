...
/**
 * The name gate, on the two names round 2 did not reach.
 *
 * Issue 283 gated a relation config's own `objectType` and
 * `relation` against the protobuf field patterns upstream applies
...
describe("Condition Name Conformance", () => {
  ...
  beforeAll(async () => {
    ...
    storeId = await fgaCreateStore("c2-names-conformance");
    authorizationModelId = await fgaWriteModel(storeId, "./c2-names/model.dsl");
  ...
  test("GAP-323: a 51-code-point condition name is stored", async () => {
    await expectBoth(name(51), "p", "refused");
  });

  test("GAP-323: a condition name holding a `#` is stored", async () => {
    await expectBoth("bad#name_c2n", "p", "refused");
  });

  test("GAP-323: a condition name holding a space is stored", async () => {
    await expectBoth("bad name_c2n", "p", "refused");
  });
  ...
  test("GAP-323: an empty condition name is stored", async () => {
    await expectBoth("", "p", "refused");
  });

  test("GAP-324: a 51-code-point parameter name is stored", async () => {
    await expectBoth("ok51_c2n", name(51), "refused");
  });

  test("GAP-324: a parameter name holding a `:` is stored", async () => {
    await expectBoth("okcolon_c2n", "bad:p", "refused");
  });

  test("the fixture's configs match its model", () => {
    expectConfigsMatchModel("./c2-names/model.dsl", fixture, {
      coverage: "complete",
    });
  });
});
