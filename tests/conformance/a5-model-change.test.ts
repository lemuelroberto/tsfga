...
describe("A5 rows outliving their type restriction (upstream corpus)", () => {
  ...
  beforeAll(async () => {
    ...
    storeId = await fgaCreateStore("a5-model-change");
    const stage0ModelId = await fgaWriteModel(
      storeId,
      "./a5-model-change/model-stage0.dsl",
    );
  ...
  test("the relation configs say what the model says", () => {
    expectConfigsMatchModel("./a5-model-change/model.dsl", fixture, {
      coverage: "complete",
    });
  });
});
