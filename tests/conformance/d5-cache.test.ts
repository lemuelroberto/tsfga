...
/**
 * Round 4, agent D5: the caching layers, probed where sharing a
 * cache entry could hand one question's answer to another.
 *
...
describe("D5 condition caches", () => {
  ...
  beforeAll(async () => {
    ...
    storeId = await fgaCreateStore("d5-cache");
    modelV1 = await fgaWriteModel(storeId, "./d5-cache/model.dsl");
  ...
  describe("one expression, three declared parameter types", () => {
    ...
    test("a warm cache entry does not excuse a failing type check", async () => {
      ...
      const bad = transformer.transformDSLToJSONObject(
        fs.readFileSync("./d5-cache/model-bad.dsl", "utf-8"),
      );
  ...
  describe("redefinition under the same name", () => {
    ...
    beforeAll(async () => {
      ...
      modelV2 = await fgaWriteModel(storeId, "./d5-cache/model-v2.dsl");
    });
  ...
  test("the relation configs say what the model says", () => {
    expectConfigsMatchModel("./d5-cache/model.dsl", fixture, {
      coverage: "complete",
    });
  });
});
