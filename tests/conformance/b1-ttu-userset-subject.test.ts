 ...
 * fixture cannot collide with another agent's rows in the
...
describe("B1 corpus — usersets over tuple-to-usersets, and cycles", () => {
  ...
  beforeAll(async () => {
    ...
    corpus = await loadCorpus(tsfgaClient, {
      slug: "b1-ttu-userset-subject",
      modelPath: "./b1-ttu-userset-subject/model.dsl",
      conditions: [
        {
          name: XCOND,
          expression: "x == '1'",
  ...
  test("the configs say what the model says", () => {
    expectConfigsMatchModel("./b1-ttu-userset-subject/model.dsl", fixture, {
      coverage: "complete",
    });
  });
});
