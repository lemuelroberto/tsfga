 ...
 *
 * `a7-model-gate.test.ts` covers the eight shapes round 1 closed
 * and the two it left open. This file walks the remainder of
 ...
 * defines, a type restriction on an undefined type) are issues
 * 153/154/155 and are deliberately absent.
 *
...
describe("Model validation rules the config gate has left", () => {
  ...
  beforeAll(async () => {
    ...
    storeId = await fgaCreateStore("d3-model-gate");
  });
  ...
  describe("reserved keywords", () => {
    for (const reserved of ["self", "this"]) {
      test(`GAP-440: '${reserved}' as an object type name is refused`, async () => {
        expect(
          await openfga({
            schema_version: "1.1",
            type_definitions: [
      ...
      test(`GAP-440: '${reserved}' as a relation name is refused`, async () => {
  ...
  describe("a rewrite naming its own relation", () => {
    test("GAP-441: a bare computed userset onto itself is refused", async () => {
      const type = "doc_d3g1";
      expect(
        await openfga({
          schema_version: "1.1",
    ...
    test("GAP-441: a union arm onto itself is refused", async () => {
      const type = "doc_d3g2";
      expect(
        await openfga({
          schema_version: "1.1",
    ...
    test("GAP-441: an intersection operand onto itself is refused", async () => {
      const type = "doc_d3g3";
      expect(
        await openfga({
          schema_version: "1.1",
    ...
    test("GAP-441: an exclusion subtracting itself is refused", async () => {
      const type = "doc_d3g4";
      expect(
        await openfga({
          schema_version: "1.1",
