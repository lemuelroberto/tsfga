...
describe("RM: matches() is refused at the model write", () => {
  ...
  /**
   * The canonical cell, and the one `c3-actions` lost its branch
   ...
   * one. It was once its own finding (GAP-380: it resolved
   * upstream and refused here, for a different reason — cel-js
   * declares only the receiver form). Both spellings are now one
   * refusal, which is the simplification dropping the feature
   * bought.
   */
