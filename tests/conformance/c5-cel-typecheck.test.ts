...
describe("CEL type-check conformance", () => {
  ...
  beforeAll(async () => {
    ...
    storeId = await fgaCreateStore("c5-cel-typecheck");
  });
  ...
  describe("GAP-381: cel-js ships extension libraries OpenFGA does not enable", () => {
    for (const cell of EXTENSIONS) {
      test(`GAP-381: ${cell.expression}`, async () => {
        const [tsfga, upstream] = await Promise.all([
          tsfgaOutcome(cell),
  ...
  describe("GAP-388: an untyped expression answers rather than refusing", () => {
    for (const cell of UNTYPED) {
      test(`GAP-388: ${cell.expression}`, async () => {
        const [tsfga, upstream] = await Promise.all([
          tsfgaOutcome(cell),
          upstreamOutcome(cell),
        ]);
