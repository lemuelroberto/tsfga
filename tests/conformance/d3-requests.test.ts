 ...
 * write path. `b5-identifiers.test.ts` covers `*` as a type and as
...
describe("Request-validation conformance", () => {
  ...
  beforeAll(async () => {
    ...
    storeId = await fgaCreateStore("d3-requests");
    authorizationModelId = await fgaWriteModel(
      storeId,
      "./d3-requests/model.dsl",
    );
  ...
  describe("a control character in the request context", () => {
    ...
    test("GAP-442: listObjects with a dirty context answers upstream", async () => {
    ...
    test("GAP-442: a dirty context key on listObjects answers upstream", async () => {
      await expectListObjectsConformance(
        storeId,
        authorizationModelId,
        tsfgaClient,
  ...
  describe("'*' as the object id", () => {
    test("GAP-443: a check whose object id is '*' is refused", async () => {
      await expectConformance(
        storeId,
        authorizationModelId,
        tsfgaClient,
    ...
    test("GAP-443: a write whose object id is '*' is refused", async () => {
      await expectWriteConformance(
        storeId,
        authorizationModelId,
        tsfgaClient,
  ...
  test("the relation configs say what the model says", () => {
    expectConfigsMatchModel("./d3-requests/model.dsl", fixture, {
      coverage: "complete",
    });
  });
});
