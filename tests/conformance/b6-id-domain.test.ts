...
import { fgaListUsers } from "./a4-rewrites/listusers.ts";
import {
  expectConformance,
...
const OUT_OF_DOMAIN: ReadonlyArray<[string, string]> = [
  ...
  // `b3-write-limits.test.ts`, where the bound is 512 bytes.
...
describe("The store's id domain", () => {
  ...
  beforeAll(async () => {
    ...
    storeId = await fgaCreateStore("b6-id-domain");
    modelId = await fgaWriteModel(storeId, "./b6-id-domain/model.dsl");
  });
  ...
  describe("the nil UUID is an ordinary id", () => {
    // Not a pin: both engines agree. It is the issue-045 property,
    // and it is why the domain checks no version nibble -- a v4
    // predicate would refuse the exact value this design frees.
