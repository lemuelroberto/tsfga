...
/**
 * Round 4, agent D5: evaluation order, where an errored branch
 * and a deciding branch are siblings.
 *
...
describe("D5 evaluation order under an errored sibling", () => {
  ...
  beforeAll(async () => {
    ...
    storeId = await fgaCreateStore("d5-order");
    authorizationModelId = await fgaWriteModel(storeId, "./d5-order/model.dsl");
  ...
  test("the relation configs say what the model says", () => {
    expectConfigsMatchModel("./d5-order/model.dsl", fixture, {
      coverage: "complete",
    });
  });
});
