...
import { setupRewrites, teardownRewrites, uuid } from "./a4-rewrites/setup.ts";
import {
...
describe("listObjects rewrite parity", () => {
  ...
  describe("refusal parity", () => {
     ...
     */
    test("GAP-060: a defined type with no tuples, undefined relation", async () => {
    ...
    test("GAP-060: an object type the model does not define", async () => {
  ...
  test("the relation configs say what the model says", () => {
    expectConfigsMatchModel("./a4-rewrites/model.dsl", fixture, {
      coverage: "complete",
    });
  });
});
