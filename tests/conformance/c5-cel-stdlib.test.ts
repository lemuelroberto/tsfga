...
/** One row per relation in `c5-cel-stdlib/model.dsl`. */
...
describe("CEL standard library conformance", () => {
  ...
  beforeAll(async () => {
    ...
    storeId = await fgaCreateStore("c5-cel-stdlib");
    modelId = await fgaWriteModel(storeId, "./c5-cel-stdlib/model.dsl");
  ...
  test("the relation configs say what the model says", () => {
    expectConfigsMatchModel("./c5-cel-stdlib/model.dsl", fixture, {
      coverage: "complete",
    });
  });
});
