 ...
 *
 * Shared by `c4-batch-check.test.ts` (checkMany against upstream's
 * BatchCheck) and `c4-list-subjects.test.ts` (listSubjects against
...
export async function setupBatch(): Promise<Fixture> {
  ...
  const storeId = await fgaCreateStore("c4-batch-conformance");
