...
/**
 * The three things round 1 changed about `listObjects`, pushed on
 * harder than the round-1 tests pushed.
 *
 * - it refuses an undefined relation or type before reading data
 *   (060),
 * - it drops a depth-exceeded candidate rather than aborting the
 *   call (170), while every *other* error still aborts,
 * - it accepts a userset subject (080).
 *
...
describe("listObjects probes", () => {
  ...
  describe("the relation gate", () => {
    ...
    test("GAP-261: an undefined subject type answers instead of refusing", async () => {
      // The subject side of the gate round 1 put on the object
    ...
    test("GAP-261: check answers false for an undefined subject type", async () => {
  ...
  describe("errors against the drop-on-depth rule", () => {
    ...
    test("GAP-260: an unreachable candidate's condition error aborts the call", async () => {
