 ...
 * context — `validateTupleWrite`, issue 042 — and nowhere else.
 */
...
describe("CEL conditions in context", () => {
  ...
  beforeAll(async () => {
    ...
    storeId = await fgaCreateStore("c5-cel-context");
    modelId = await fgaWriteModel(storeId, "./c5-cel-context/model.dsl");
   ...
   */
  describe("GAP-386: control characters in a check's request context", () => {
    test("GAP-386: a newline", async () => {
      await ctl({ s: "a\nb" }, "refused");
    });

    test("GAP-386: a tab", async () => {
      await ctl({ s: "a\tb" }, "refused");
    });

    test("GAP-386: a carriage return", async () => {
      await ctl({ s: "a\rb" }, "refused");
    });

    test("GAP-386: a NUL", async () => {
      await ctl({ s: "a b" }, "refused");
    });

    test("GAP-386: DEL", async () => {
      await ctl({ s: "ab" }, "refused");
    });

    test("GAP-386: a C1 control character", async () => {
      await ctl({ s: "ab" }, "refused");
    });

    test("GAP-386: a control character nested in a list", async () => {
  ...
  test("the relation configs say what the model says", () => {
    expectConfigsMatchModel("./c5-cel-context/model.dsl", fixture, {
      coverage: "complete",
    });
  });
});
