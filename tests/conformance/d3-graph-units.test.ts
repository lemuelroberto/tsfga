 ...
 * the exclusion counterpart of the rule issue 200 pinned on a
 * union: an error on one arm is discarded when the other arm
...
describe("Resolver unit shapes from internal/graph", () => {
  ...
  beforeAll(async () => {
    ...
    storeId = await fgaCreateStore("d3-graph-units");
    authorizationModelId = await fgaWriteModel(
      storeId,
      "./d3-graph-units/model.dsl",
    );
  ...
  test("the relation configs say what the model says", () => {
    expectConfigsMatchModel("./d3-graph-units/model.dsl", fixture, {
      coverage: "complete",
    });
  });
});
