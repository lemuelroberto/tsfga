...
describe("Slack Connect Model Conformance", () => {
  ...
  beforeAll(async () => {
    ...
    // === Tuples (mirroring ./d4-connect/tuples.yaml) ===
  ...
  test("15: the guest is internal, so internal-only keeps him", async () => {
    ...
    // too, and a context that cannot evaluate it is issue 200's
    // pinned shape rather than anything this fixture is about.
  ...
  test("the relation configs say what the model says", () => {
    expectConfigsMatchModel("./d4-connect/model.dsl", fixture, {
      coverage: "complete",
    });
  });
});
