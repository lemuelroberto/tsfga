 ...
 *
 * `c5-cel-arith-surface.test.ts` enumerated the *arithmetic*
 ...
 * **granting** direction:
 *
 * - **`duration` has no magnitude check** (issue 420). Upstream's
 ...
 *   instead of refusing it** (issue 421). `2026-02-30` becomes
...
/** One row per relation in `d2-coercion/model.dsl`. */
...
describe("Context coercion conformance", () => {
  ...
  beforeAll(async () => {
    ...
    storeId = await fgaCreateStore("d2-coercion");
    modelId = await fgaWriteModel(storeId, "./d2-coercion/model.dsl");
   ...
   */
  describe("GAP-420: a duration's magnitude is never checked", () => {
    test("GAP-420: a duration past int64 nanoseconds", async () => {
      await check("durctx_d2", { d: DUR_OVER }, "refused");
    });

    test("GAP-420: one nanosecond past the maximum", async () => {
      await check("durctx_d2", { d: DUR_OVER_BY_ONE }, "refused");
    });

    test("GAP-420: a negative duration past int64 nanoseconds", async () => {
      await check("durctx_d2", { d: `-${DUR_OVER}` }, "refused");
    });

    test("GAP-420: unit terms that overflow only when summed", async () => {
      // Each term is inside the range; Go overflows adding them.
      await check("durctx_d2", { d: "2400000h2400000h" }, "refused");
    });

    test("GAP-420: a nanosecond count past int64", async () => {
      await check("durctx_d2", { d: "9223372036854775808ns" }, "refused");
    });

    test("GAP-420: the same value as a list<duration> element", async () => {
      await check("durlist_d2", { ds: [DUR_OVER] }, "refused");
    });

    test("GAP-420: the same value as a map<duration> value", async () => {
      await check("durmap_d2", { dm: { a: DUR_OVER } }, "refused");
    });

    test("GAP-420: the write gate stores what upstream refuses", async () => {
   ...
   */
  describe("GAP-421: a timestamp is normalised rather than validated", () => {
    test("GAP-421: a day past the end of the month", async () => {
      await check("tsctx_d2", { t: "2026-02-30T00:00:00Z" }, "refused");
    });

    test("GAP-421: February 29 in a common year", async () => {
      await check("tsctx_d2", { t: "2026-02-29T00:00:00Z" }, "refused");
    });

    test("GAP-421: the 31st of a thirty-day month", async () => {
      await check("tsctx_d2", { t: "2026-04-31T00:00:00Z" }, "refused");
    });

    test("GAP-421: hour 24", async () => {
      await check("tsctx_d2", { t: "2026-01-01T24:00:00Z" }, "refused");
    });

    test("GAP-421: the same value as a list<timestamp> element", async () => {
      await check("tslist_d2", { ts: ["2026-02-30T00:00:00Z"] }, "refused");
    });

    test("GAP-421: the write gate stores what upstream refuses", async () => {
      await write("tsctx_d2", { t: "2026-02-30T00:00:00Z" }, "refused");
    });

    test("GAP-421: hour 24 on the write gate", async () => {
   ...
   */
  describe("GAP-423: a zone offset upstream reads and tsfga does not", () => {
    test("GAP-423: an offset with minute 60", async () => {
  ...
  test("the relation configs say what the model says", () => {
    expectConfigsMatchModel("./d2-coercion/model.dsl", fixture, {
      coverage: "complete",
    });
  });
});
