 ...
 *
 * Round 2 replaced `JSON.stringify().length` with a hand-rolled
 * `proto.Size` of the `google.protobuf.Struct` the context becomes
 * (issue 242), calibrated on `{"s": "<n bytes>"}` alone. That one
...
describe("Condition Context Size Conformance", () => {
  ...
  beforeAll(async () => {
    ...
    storeId = await fgaCreateStore("c2-struct-size-conformance");
    authorizationModelId = await fgaWriteModel(
      storeId,
      "./c2-struct-size/model.dsl",
    );
  });
  ...
  test("the fixture's configs match its model", () => {
    expectConfigsMatchModel("./c2-struct-size/model.dsl", fixture, {
      coverage: "complete",
    });
  });
});
