...
/**
 * Round 3 put a name gate on `writeConditionDefinition` (issues
 * 323 / 324), reusing the relation-name predicate: the character
 * class `[^:#@\s]` and a bound of 50 code points.
 *
 ...
 * that nothing here measured, so it is measured here.
 *
...
describe("condition name gate: over-refusal sweep", () => {
  ...
  beforeAll(async () => {
    ...
    storeId = await fgaCreateStore("d1-names");
  });
