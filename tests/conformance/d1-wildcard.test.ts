...
/**
 * `CheckTuples.wildcard` became a list in round 3 (issue 342), so
 * a contextual `user:*` row no longer hides the stored one. This
...
describe("wildcard rows joined across stored and contextual", () => {
  ...
  beforeAll(async () => {
    ...
    storeId = await fgaCreateStore("d1-wildcard");
    modelId = await fgaWriteModel(storeId, "./d1-wildcard/model.dsl");
  ...
  test("relation configs match the DSL", () => {
    expectConfigsMatchModel("./d1-wildcard/model.dsl", fixture, {
      coverage: "complete",
    });
  });
});
