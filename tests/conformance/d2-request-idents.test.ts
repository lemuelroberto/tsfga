...
describe("Request identifier conformance", () => {
  ...
  beforeAll(async () => {
    ...
    storeId = await fgaCreateStore("d2-request-idents");
    modelId = await fgaWriteModel(storeId, "./d2-idents/model.dsl");
  ...
  describe("GAP-422: the object id is not validated", () => {
    for (const [name, char] of MALFORMED) {
      test(`GAP-422: an object id containing ${name}`, async () => {
        await check({ objectId: `${uuid("doc")}${char}` }, "refused");
      });
    }

    test("GAP-422: an empty object id", async () => {
      // `doc_d2i:` has an id of length zero, which `IsValidObject`
      // refuses.
      await check({ objectId: "" }, "refused");
    });

    test("GAP-422: an object id past the wire length limit", async () => {
  ...
  describe("GAP-422: the subject id is validated only for ':' and '#'", () => {
    for (const [name, char] of MALFORMED) {
      test(`GAP-422: a subject id containing ${name}`, async () => {
        await check({ subjectId: `${uuid("alice")}${char}` }, "refused");
      });
    }

    test("GAP-422: an empty subject id", async () => {
      await check({ subjectId: "" }, "refused");
    });

    test("GAP-422: a subject id past the wire length limit", async () => {
  ...
  describe("GAP-422: listObjects carries the same hole", () => {
    test("GAP-422: a subject id containing a control character", async () => {
    ...
    test("GAP-422: a subject id containing a space", async () => {
      await expectListObjectsRefusal({
        objectType: "doc_d2i",
        relation: "viewer",
        subjectType: "user_d2i",
   ...
   */
  describe("GAP-422: listSubjects carries the same hole", () => {
    ...
    test("GAP-422: an object id containing a space", async () => {
      const [tsfga, openfga] = await outcomes(`${uuid("doc")} `);
      expect(openfga).toBe("refused");
      expect(tsfga).toBe(openfga);
    });
   ...
   */
  describe("GAP-422: checkMany reports a decision, not a failure", () => {
    ...
    test("GAP-422: an item whose object id holds a control char", async () => {
      const [tsfga, openfga] = await outcomes(
        `${uuid("doc")}${String.fromCharCode(1)}`,
        uuid("alice"),
      );
    ...
    test("GAP-422: an item whose subject id holds a control char", async () => {
      const [tsfga, openfga] = await outcomes(
        uuid("doc"),
  ...
  test("the relation configs say what the model says", () => {
    expectConfigsMatchModel("./d2-idents/model.dsl", fixture, {
      coverage: "complete",
    });
  });
});
