...
import { cfg, ids } from "./b1-corpus.ts";
import {
...
describe("B1 a condition error spreading from an unrelated object", () => {
  ...
  beforeAll(async () => {
    ...
    storeId = await fgaCreateStore("b1-condition-spread");
    authorizationModelId = await fgaWriteModel(
      storeId,
      "./b1-condition-spread/model.dsl",
    );
  ...
  test("the configs say what the model says", () => {
    expectConfigsMatchModel("./b1-condition-spread/model.dsl", fixture, {
      coverage: "complete",
    });
  });
});
