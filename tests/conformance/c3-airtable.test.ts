...
describe("Airtable Model Conformance", () => {
  ...
  beforeAll(async () => {
    ...
    // === Tuples (mirroring ./c3-airtable/tuples.yaml) ===
    ...
    storeId = await fgaCreateStore("c3-airtable");
    authorizationModelId = await fgaWriteModel(
      storeId,
      "./c3-airtable/model.dsl",
    );
  ...
  test("the relation configs say what the model says", () => {
    expectConfigsMatchModel("./c3-airtable/model.dsl", fixture, {
      coverage: "complete",
    });
  });
});
