...
import { MATRIX_HELPERS, MATRIX_MOVED } from "./c1-matrix/configs.ts";
import {
  type CheckAssertion,
  checkRequest,
  createCaseStore,
...
/**
 * The three things round 2 changed that only their own tests
 * currently guard, asked again at the bottom of a four-level
 * composition:
 *
 * 1. `listObjects` now classifies a condition error by **which
 *    read raised it** — a subject-naming read aborts, a scan read
...
const TUPLES: MatrixTuple[] = [
  ...
  // read above and this one — which is exactly the pair round 2's
...
describe("c1: round-2 behaviour under a four-level composition", () => {
  ...
  describe("listObjects with a context the condition cannot use", () => {
    for (const [name, context] of [
      ...
      test(`GAP-301: ${name}: nothing is granted`, async () => {
        // `ce_carol` reaches nothing at all — but the scan still
        // visits `ce_1` and `ce_4`, whose conditions cannot be
        // evaluated. Whether
        // the answer is `[ce_2]` or a refusal is exactly the
        // classification round 2 introduced.
        expectSame(
          await listObjectsOutcomes(
            storeId,
            authorizationModelId,
  ...
  test("configs match the model", () => {
    expectConfigsMatchModel("./c1-matrix/model.dsl", fixture, {
      coverage: "complete",
