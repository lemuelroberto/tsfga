...
import { MATRIX_HELPERS, MATRIX_MOVED } from "./b4-matrix/configs.ts";
import {
 ...
 * which `listObjects` only learned to take in round 1. Upstream
 * asserts them throughout, so they are the ones to watch.
 */
...
describe("listObjects matrix — usersets", () => {
  ...
  test("configs match the model", () => {
    expectConfigsMatchModel("./b4-matrix/model.dsl", fixture, {
      coverage: "complete",
