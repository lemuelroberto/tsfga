 ...
 *
 * Deliberately a sibling of `b4-matrix/harness.ts` rather than an
 ...
 * use. Every id is under this agent's `d4d0` prefix.
 */
...
export async function createCaseStore(
  ...
  const storeId = await fgaCreateStore(`c1-matrix-${name}`);
