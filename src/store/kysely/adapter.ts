...
import type { TupleStore } from "../interface.ts";
...
export class KyselyTupleStore implements TupleStore {
	...
	async findRelationConfig(
		...
		const row = await this.db
			.selectFrom("fga.relation_configs")
			.selectAll()
			.where("object_type", "=", objectType)
			.where("relation", "=", relation)
			.executeTakeFirst();
	...
	async findConditionDefinition(
		...
		const row = await this.db
			.selectFrom("fga.condition_definitions")
			.selectAll()
			.where("name", "=", name)
			.executeTakeFirst();
	...
	async insertTuple(tuple: AddTupleRequest): Promise<void> {
		...
		await sql`
			INSERT INTO fga.tuples (object_type, object_id, relation, subject_type, subject_id, subject_relation, condition_name, condition_context)
			VALUES (
				${tuple.objectType},
				${tuple.objectId}::uuid,
				${tuple.relation},
	...
	async upsertRelationConfig(config: RelationConfig): Promise<void> {
		await this.db
			.insertInto("fga.relation_configs")
			.values({
	...
	async deleteRelationConfig(
		...
		const result = await this.db
			.deleteFrom("fga.relation_configs")
			.where("object_type", "=", objectType)
			.where("relation", "=", relation)
			.executeTakeFirst();
	...
	async upsertConditionDefinition(
		...
		await this.db
			.insertInto("fga.condition_definitions")
			.values({
				name: condition.name,
	...
	async deleteConditionDefinition(name: string): Promise<boolean> {
		const result = await this.db
			.deleteFrom("fga.condition_definitions")
			.where("name", "=", name)
			.executeTakeFirst();
